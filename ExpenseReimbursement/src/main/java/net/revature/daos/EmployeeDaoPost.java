...
public class EmployeeDaoPost implements EmployeeDAO {
    ...
    public int create(Employee engineer) {
        ...
        String sql = "insert into employee (id, full_name, dept_id, username, pass_word)" +
                "values (default, ?, ?, ?, ?)";
        ...
        try {
            ...
            employeeStatement.setString(1, engineer.getUsername());
            employeeStatement.setString(2,  engineer.getPassword());
            employeeStatement.setInt(3, engineer.getDeptId());
            employeeStatement.setString(4, engineer.getFullName());
    ...
    public Employee getById(int id) {
        ...
        try {
            ...
            if (results.next()) {
            	...
            	engineer.setId(id);
            	engineer.setFullName(results.getString("full_name"));
    ...
    public void update(Employee updatedObj) {
        Connection connection = connFactory.getConnection();
        String sql = "update employee set pass_word=?, username=?, full_name=?, dept_id=?, " +
                "id=?, request_id=?";
        try {
            ...
            updated.setString(3, updatedObj.getFullName());
	...
	public List<Employee> getAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				engineer.setFullName(fullName.substring(0, fullName.indexOf(' ')));
